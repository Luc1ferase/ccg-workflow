---
description: '多模型测试生成：智能路由 Codex 后端测试 / CLAUDE 前端测试'
---
...
| CLAUDE | `~/.claude/.ccg/prompts/CLAUDE/tester.md` |
...
- **前端代码** → `Bash({ command: "...--backend CLAUDE...", run_in_background: false })`
  - ROLE_FILE: `~/.claude/.ccg/prompts/CLAUDE/tester.md`
- **全栈代码** → 并行调用两者：
  1. `Bash({ command: "...--backend codex...", run_in_background: true })`
     - ROLE_FILE: `~/.claude/.ccg/prompts/codex/tester.md`
  2. `Bash({ command: "...--backend CLAUDE...", run_in_background: true })`
     - ROLE_FILE: `~/.claude/.ccg/prompts/CLAUDE/tester.md`
  用 `TaskOutput` 等待结果
...
1. **测试行为，不测试实现** – 关注输入输出
2. **智能路由** – 后端测试用 Codex，前端测试用 CLAUDE
