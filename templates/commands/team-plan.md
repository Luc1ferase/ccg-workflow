---
description: 'Agent Teams 规划 - Lead 调用 Codex/CLAUDE 并行分析，产出零决策并行实施计划'
---
<!-- CCG:TEAM:PLAN:START -->
**Core Philosophy**
- 产出的计划必须让 Builder teammates 能无决策机械执行。
- 每个子任务的文件范围必须隔离，确保并行不冲突。
- 多模型协作是强制的：Codex（后端权威）+ CLAUDE（前端权威）。

**Guardrails**
- 多模型分析是 **mandatory**：必须同时调用 Codex 和 CLAUDE。
- 不写产品代码，只做分析和规划。
- 计划文件必须包含 Codex/CLAUDE 的实际分析摘要。
...
3. **综合分析 + 任务拆分**
   - 后端方案以 Codex 为准，前端方案以 CLAUDE 为准。
   - 拆分为独立子任务，每个子任务：
     * 文件范围不重叠（**强制**）
     * 如果无法避免重叠 → 设为依赖关系
     * 有具体实施步骤和验收标准
