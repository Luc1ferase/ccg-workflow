---
description: '多模型调试：Codex 后端诊断 + CLAUDE 前端诊断，交叉验证定位问题'
---
...
**CLAUDE 前端诊断**：
```bash
~/.claude/bin/codeagent-wrapper --backend CLAUDE - "$(pwd)" <<'EOF'
ROLE_FILE: ~/.claude/.ccg/prompts/CLAUDE/debugger.md
<TASK>
需求：<增强后的需求>
上下文：<错误日志、堆栈信息、复现步骤>
</TASK>
...
| CLAUDE | `~/.claude/.ccg/prompts/CLAUDE/debugger.md` |

**并行调用**：
1. 使用 `Bash` 工具，设置 `run_in_background: true` 和 `timeout: 600000`（10 分钟）
2. 同时发起两个后台任务（Codex + CLAUDE）
...
2. **CLAUDE 前端诊断**：`Bash({ command: "...--backend CLAUDE...", run_in_background: true })`
   - ROLE_FILE: `~/.claude/.ccg/prompts/CLAUDE/debugger.md`
   - OUTPUT：诊断假设（按可能性排序），每个假设包含原因、证据、修复建议
...
1. **用户确认** – 修复前必须获得确认
2. **信任规则** – 后端问题以 Codex 为准，前端问题以 CLAUDE 为准
3. 外部模型对文件系统**零写入权限**
